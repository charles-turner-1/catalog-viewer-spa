  ...
  <div class="datastore-detail-container">
    ...
    >
      <div class="flex items-center">
        ...
        <strong class="text-yellow-700 dark:text-yellow-300"
          >Alpha Software:</strong
        >
      </div>
      <p class="text-yellow-700 dark:text-yellow-300 mt-1">
        The intake catalog interface is currently in alpha and under active
        development. Features and functionality may change in future releases.
      </p>
    </div>
    ...
    <div v-if="!loading && !error">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
        ...
        >
          <div
            v-for="(options, column) in filterOptions"
            :key="column"
            class="flex flex-col"
          >
            <label
...
const filterOptions = computed(
  () => cachedDatastore.value?.filterOptions || {},
);
...
const loadDatastore = async () => {
  ...
  try {
    ...
    const datastoreCache = await catalogStore.loadDatastore(
      datastoreName.value,
    );
